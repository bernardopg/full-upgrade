...
# Réplica off-site do snapshot Timeshift via Restic+rclone (criptografada).
: "${TIMESHIFT_CLOUD_BACKUP:=0}"  # 1 = envia o snapshot mais recente após criá-lo
: "${TIMESHIFT_CLOUD_KEEP:=3}"    # versões remotas mais recentes a manter
: "${TIMESHIFT_CLOUD_REPOSITORY:=rclone:onedrive:full-upgrade/timeshift}"
: "${TIMESHIFT_CLOUD_PASSWORD_FILE:=${FU_CONFIG_DIR}/restic-timeshift-password}"
: "${TIMESHIFT_CLOUD_RCLONE_CONFIG:=${XDG_CONFIG_HOME:-${HOME}/.config}/rclone/rclone.conf}"
: "${TIMESHIFT_CLOUD_EXCLUDE_FILE:=${FU_CONFIG_DIR}/restic-timeshift-excludes}"
: "${TIMESHIFT_CLOUD_PROGRESS_INTERVAL:=60}"  # segundos entre heartbeats do upload
...
SNAPSHOT_KEEP
TIMESHIFT_CLOUD_BACKUP
TIMESHIFT_CLOUD_KEEP
TIMESHIFT_CLOUD_REPOSITORY
TIMESHIFT_CLOUD_PASSWORD_FILE
TIMESHIFT_CLOUD_RCLONE_CONFIG
TIMESHIFT_CLOUD_EXCLUDE_FILE
...
load_config() {
  ...
  [[ -z "$ANTIGRAVITY_IDE_BIN" ]] && ANTIGRAVITY_IDE_BIN="$(command -v antigravity-ide 2>/dev/null || true)"
  ...
  export SNAPSHOT_MIN_FREE_GIB SNAPSHOT_KEEP TIMESHIFT_CLOUD_BACKUP TIMESHIFT_CLOUD_KEEP
  ...
  export FULL_UPGRADE_DMS_PLUGINS_IGNORE
...
# ── Réplica Timeshift criptografada no OneDrive (Restic+rclone) ──
...
TIMESHIFT_CLOUD_PROGRESS_INTERVAL=60
...
# ATIVA do arquivo atual cabe inteira no .pacnew (o caso comum: o usuário só
# descomentou linhas e o pacote só mexeu em comentários/opções novas). O merge
# parte do .pacnew e reinsere as linhas ativas byte a byte; se qualquer diretiva
# em vigor mudasse, o arquivo é recusado e segue como decisão manual. Sempre
# grava backup <arquivo>.full-upgrade.bak.<timestamp>. /etc/sudoers, passwd,
# shadow, group, gshadow, fstab e crypttab nunca entram no merge automático.
# Nunca roda sob --mode doctor/--dry-run/--no-repair.
AUTO_MERGE_PACNEW=0

# ── Política de Secure Boot no --audit ──
# 0 = Secure Boot desabilitado é postura informativa (default; nem toda máquina
# usa Secure Boot). 1 = classifica como severidade média em ambientes estritos.
...
show_config() {
  ...
  _cfg_kv "TIMESHIFT_CLOUD_BACKUP" "$TIMESHIFT_CLOUD_BACKUP"
  _cfg_kv "TIMESHIFT_CLOUD_KEEP" "$TIMESHIFT_CLOUD_KEEP"
  _cfg_kv "TIMESHIFT_CLOUD_REPOSITORY" "$TIMESHIFT_CLOUD_REPOSITORY"
  _cfg_kv "TIMESHIFT_CLOUD_PASSWORD_FILE" "$TIMESHIFT_CLOUD_PASSWORD_FILE"
  _cfg_kv "TIMESHIFT_CLOUD_RCLONE_CONFIG" "$TIMESHIFT_CLOUD_RCLONE_CONFIG"
  _cfg_kv "TIMESHIFT_CLOUD_EXCLUDE_FILE" "$TIMESHIFT_CLOUD_EXCLUDE_FILE"
  _cfg_kv "TIMESHIFT_CLOUD_PROGRESS_INTERVAL" "$TIMESHIFT_CLOUD_PROGRESS_INTERVAL"
  ...
  _cfg_kv "ANTIGRAVITY_IDE_BIN" "$ANTIGRAVITY_IDE_BIN" "<não encontrado>"
