...
- `git_tracking_ref` — the upstream ref must be *verified to exist*, not just read from config. With `--prune` above, `branch.<name>.merge` can still name a branch that is already gone; without the check, "behind" is measured against a nonexistent ref (`rev-list` fails → 0) and the plugin freezes with no updates and no warning. Falls back to `origin/HEAD`.
- `git_remote_gone` — fetch failures have **three** classes, not two. Besides transient network (warn) and genuine operational failure (fail), there is *permanently unreachable upstream*: repo deleted, made private, renamed without redirect, or credentials revoked. It answers `Repository not found` on every run and never recovers on its own, so classifying it as `fail` broke the step in 100% of runs forever, with no action the tool itself could take. Match `GIT_REMOTE_GONE_RE` **before** `NETWORK_TRANSIENT_RE` and return `RC_TODO` with the remediation (fix `remote set-url` or drop the orphan clone). Keep hard `fail` for errors that really indicate a defect.
