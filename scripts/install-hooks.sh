...
# de um commit quebrado virar o HEAD público. É deliberadamente rápido (~13s):
# sintaxe + shellcheck + build. A suíte bats completa continua no CI.
#
...
echo "  Roda scripts/preflight.sh (~13s). Pule com: git push --no-verify"
