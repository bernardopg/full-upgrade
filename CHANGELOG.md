...
### Corrigido
- **Journal/Doctor:** executáveis em `COREDUMP_IGNORE_EXE` também filtram suas linhas do `systemd-coredump` no journal, e a corrida transitória `Failed to initialize pidref` não gera aviso.
- **Journal/Doctor:** ciclos de ordenação do systemd agora recebem orientação acionável para corrigir a relação `After=`/`WantedBy=`.
...
- **Durações alinhadas em coluna no resumo e no output ao vivo.** Os nomes dos
  steps são preenchidos até uma coluna comum (largura = maior nome, limitada a
  `[24,50]`) para que as durações/motivos `(…)` fiquem alinhados — tanto nas
  linhas de status ao vivo (✔/⚠/→/✘/⊘) quanto no resumo final. Novo helper puro
  `ui_pad`, ciente de UTF-8 (conta caracteres, não bytes — nomes acentuados PT-BR
  alinham certo), e largura memoizada `_step_name_width` derivada do catálogo.
  Testes bats inclusos.
...
- **Step Doctor: apps manuais (fora de pacote)** (`doctor_manual_apps`) —
  ...
  quais já têm step de atualização dedicado. Não executa binários desconhecidos
  (evita abrir GUIs). Helper puro `_manual_apps_has_step` com testes bats.
