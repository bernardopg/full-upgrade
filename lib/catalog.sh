...
Atualizar plugins do Claude Code|ai|ai,claude,plugins,update,network|mutating|600|claude,python3|update_claude_plugins|Atualiza os marketplaces de plugins do Claude Code e cada plugin de escopo user (plugin update --json); comando de instalação alterado vira TODO, nunca é aceito sozinho.
...
Auto-remediar config MCP do Codex|autofix|mcp,ai,config|mutating|60||autofix_codex_mcp_toml|Sob AUTO_FIX_CODEX_MCP=1 (padrão), remove de ~/.codex/config.toml tabelas duplicadas cujo conteúdo é idêntico à primeira ocorrência (caso típico: [mcp_servers.headroom_memory] reanexado por 'headroom init codex'), com backup e rollback; duplicata divergente vira todo.
